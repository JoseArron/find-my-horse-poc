...
import { IndexedResult, type SageMakerObjectDetectionResponse } from "./types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const results = dbResult.Item as IndexedResult;
    ...
    return NextResponse.json({
      ...
      labels: [
        {
          name: results.horse_name,
          confidence: 100,
          boundingBox: {
            left: results.annotations[0].left,
            top: results.annotations[0].top,
            width: results.annotations[0].width,
            height: results.annotations[0].height,
          },
        },
      ],
      totalLabels: results.annotations.length,
    });
  } catch (error) {
